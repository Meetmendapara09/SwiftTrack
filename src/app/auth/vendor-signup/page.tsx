...
export default function VendorSignupPage() {
  ...
  const handleSubmit = async (data: { name: string, email: string, password: string }) => {
    ...
    const { success, error, message } = await signUp({ ...data, role: 'vendor' });
    ...
    if (success) {
      ...
      toast({ 
        title: "Signup Failed", 
        description: message || error?.message || "Could not create vendor account. Please try again.", 
        variant: "destructive",
        duration: 7000,
      });
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-[calc(100vh-12rem)] py-12 animate-in fade-in-0 slide-in-from-bottom-8 duration-700">
        ...
        title="Vendor Signup"
        description="Join SwiftTrack to reach more customers and streamline your sales."
        loginLink="/auth/vendor-login"
